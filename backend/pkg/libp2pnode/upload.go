...
func GetPbContentDataFromModelsContentData(instanceId string, data *models.ContentData) *pb.ContentData {
...
func (e *UploadProtocol) processUploadRequest(req *pb.UploadRequest) *pb.UploadResponse {
	...
	previewUrl, previewFileType, err := GetPreviewFrom(req.UploadData)
	if err != nil {
		return e.getUploadResponseWithError(req, http.StatusInternalServerError, commontools.Errorf(err, "GetPreviewFrom failed"))
	}
	...
	return &pb.UploadResponse{
		...
		ContentData:          GetPbContentDataFromModelsContentData(instanceId, &contentData),
	}
}
